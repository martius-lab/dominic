...
class A1VanillaEnvCfg(BaseEnvCfg):
    seed = 42
    ...
    class asset(BaseEnvCfg.asset):
        file = '{root}/resources/robots/a1/urdf/a1.urdf'
        name = "a1"
        foot_name = "foot"
    ...
    class rewards(BaseEnvCfg.rewards):
        class terms:  # [group, sigma]
            lin_vel_x = ["task", np.sqrt(0.02)]
            lin_vel_y = ["task", np.sqrt(0.02)]
            ang_vel_z = ["task", np.sqrt(0.2)]

            base_height = ["tracking", np.sqrt(0.05)]

            action_rate = ["regularizer", np.sqrt(10.0)]
...
class A1VanillaTrainCfg(BaseTrainCfg):
