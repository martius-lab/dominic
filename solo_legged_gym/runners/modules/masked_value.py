...
class MaskedValue(nn.Module):
    def __init__(self,
        ...
        if hidden_dims is None:
            hidden_dims = [256, 256]
        if kwargs:
            print("MaskedValue.__init__ got unexpected arguments, which will be ignored: " + str(
                [key for key in kwargs.keys()]))
        super(MaskedValue, self).__init__()
        ...
        for la in range(self.num_hidden_dim):
            if la == self.num_hidden_dim - 1:
                self.layers.append(nn.Linear(hidden_dims[la], 1).to(self.device))
            else:
